...
function ResumePage({ resumeData, setResumeData, showToast, isPro, setPage, totalScans, onScanComplete }) {
  ...
  const downloadReport = () => {
    ...
    const scoreColor = resumeData.score >= 75 ? "#2d8a4e" : resumeData.score >= 55 ? "#c2790a" : "#c53030";
    const breakdownRows = Object.entries(resumeData.breakdown || {}).map(([k, v]) => {
    const s = v?.score ?? 0;
    const c = s >= 75 ? "#2d8a4e" : s >= 55 ? "#c2790a" : "#c53030";
    return `<div class="bar-row"><div class="bar-label"><span>${v?.label || k}</span><span style="font-weight:700;color:${c}">${s}/100</span></div><div class="bar-bg"><div class="bar-fill" style="width:${s}%;background:${c}"></div></div></div>`;
  }).join("");
  const skillTags = (resumeData.skills || []).map(s => `<span class="tag tag-green">✓ ${s}</span>`).join("");
  const missingTags = (resumeData.missing || []).map(s => `<span class="tag tag-red">✗ ${s}</span>`).join("");
  const impRows = (resumeData.improvements || []).map((imp, i) => `<div class="imp"><strong>#${i+1}</strong> ${imp}</div>`).join("");

  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>CareerLens Resume Report — ${resumeData.name}</title>
<style>
  ...
  .bar-label{display:flex;justify-content:space-between;font-size:.8rem;margin-bottom:3px}
  .bar-bg{height:8px;background:#e5e2de;border-radius:99px;overflow:hidden}
...
<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:24px;padding-bottom:20px;border-bottom:2px solid #1a1916">
  ...
  <div style="text-align:center">
    <div class="score">${resumeData.score}</div>
    <div style="font-size:.7rem;color:#9a958f;font-weight:600">/100 Overall</div>
  </div>
</div>
<h2>Score Breakdown</h2>${breakdownRows}
<h2>Skills Found</h2><div>${skillTags}</div>
<h2>Missing Skills</h2><div>${missingTags}</div>
<h2>Top Improvements</h2>${impRows}
<h2>Summary</h2><p style="font-size:.85rem;color:#3a3632">${resumeData.summary}</p>
<div class="footer">Generated by CareerLens · carrerlens.com · Scan your resume again to track improvement</div>
</body></html>`;
      ...
      <div className="page-header">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-ghost btn-sm" onClick={downloadReport} style={{ gap: 5 }}>
              ↓ PDF Report
            </button>
