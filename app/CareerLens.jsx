...
${Object.entries(resumeData.breakdown || {}).map(([k,v]: [string,any]) => {
  const s = v?.score ?? 0;
...
<h2>Skills Found</h2>
<div>${(resumeData.skills || []).map((s: string) => `<span class="tag tag-green">✓ ${s}</span>`).join("")}</div>

<h2>Missing Skills</h2>
<div>${(resumeData.missing || []).map((s: string) => `<span class="tag tag-red">✗ ${s}</span>`).join("")}</div>

<h2>Top Improvements</h2>
${(resumeData.improvements || []).map((imp: string, i: number) => `<div class="imp"><strong>#${i+1}</strong> ${imp}</div>`).join("")}

<h2>Summary</h2>
<p style="font-size:.85rem;color:#3a3632">${resumeData.summary}</p>
